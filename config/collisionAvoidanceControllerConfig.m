...
function cfg = localDefaults()
    ...
    cfg.encounter = struct("barrierFraction", 1.0, ...
        ...
        "maximumNominalRefinements",5,"nominalLinearizationReserve",0.02, ...
        "anticipationReserveFractionMaximum",0.25, ...
        "safetyMarginPolicy","maximize", ...
        "maneuverSelectionPolicy", "retainFeasibleManeuver", ...
...
function localValidate(cfg)
    ...
    if ~isscalar(string(cfg.encounter.safetyMarginPolicy)) ...
            || ~any(string(cfg.encounter.safetyMarginPolicy)==["maximize","performance"])
        error("collisionAvoidanceController:invalidConfiguration","Unknown safety-margin policy.");
    end
    validateattributes(cfg.encounter.maximumNominalRefinements,{'double'}, ...
        {'scalar','real','finite','integer','nonnegative'});
    validateattributes(cfg.encounter.nominalLinearizationReserve,{'double'}, ...
        {'scalar','real','finite','nonnegative'});
    validateattributes(cfg.encounter.anticipationReserveFractionMaximum,{'double'}, ...
        {'scalar','real','finite','>=',0,'<=',1});
    validateattributes(cfg.encounter.maximumGeometryRebuilds, {'double'}, ...
        {'scalar','real','finite','integer','nonnegative'});
    ...
    validateattributes(cfg.controller.certifiedSteps,{'double'},{'scalar','real','positive'});
    validateattributes(cfg.controller.inputDelaySteps,{'double'},{'scalar','integer','>=',0,'<=',1});
    if cfg.controller.horizonSteps<=cfg.controller.inputDelaySteps ...
            || cfg.controller.certifiedSteps<=cfg.controller.inputDelaySteps
        error("collisionAvoidanceController:invalidConfiguration", ...
            "Prediction and certification must include the delayed command's complete held interval.");
    end
    ...
    if isfinite(cfg.controller.certifiedSteps) && cfg.controller.certifiedSteps~=fix(cfg.controller.certifiedSteps)
        error("collisionAvoidanceController:invalidConfiguration","certifiedSteps must be a positive integer or Inf.");
    end
    validateattributes(cfg.encounter.barrierFraction, {'double'}, ...
        {'scalar', 'finite', '>', 0, '<=', 1});
    if ~isscalar(string(cfg.encounter.maneuverSelectionPolicy)) ...
            || ~any(string(cfg.encounter.maneuverSelectionPolicy)==["retainFeasibleManeuver","bestFeasibleObjective"])
        error("collisionAvoidanceController:invalidConfiguration","Invalid maneuver selection policy.");
    end
