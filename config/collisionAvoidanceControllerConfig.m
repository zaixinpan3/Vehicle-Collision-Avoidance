...
function cfg = localDefaults()
    ...
    cfg.clf = struct( ...
        ...
        "samplePoints", "endpoints");
    % samplePoints selects where the sampled-data CLF decrease is imposed
    % and verified: "endpoints" at the two ends of every tube cell,
    % "controlPoints" at every Bernstein control point of every cell.
...
function localValidate(cfg)
    ...
    if ~ismember(string(cfg.clf.samplePoints),["endpoints","controlPoints"])
        error("collisionAvoidanceController:invalidConfiguration", ...
            "clf.samplePoints must be ""endpoints"" or ""controlPoints"".");
    end
