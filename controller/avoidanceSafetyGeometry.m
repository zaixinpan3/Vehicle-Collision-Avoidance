...
% evaluation, with no trajectory optimization or alternative solver start.
    ...
    replaced = 0;
    for nodeIdx = 1:count
        if nodeIdx == count && ~isempty(carried) && (dissipative || model.hasTarget)
            ...
            if model.hasTarget
                % The newly appended target node has not been certified by
                % the previous finite forecast. Always build its fresh row.
                collision.nodes(nodeIdx) = localTargetNode(empty, model, prediction, ...
                    state(:, nodeIdx), frames(nodeIdx), nodeIdx);
            end
            for boundaryIdx = 1:numel(road)
...
function node = localTargetNode(node, model, prediction, state, frame, nodeIdx)
    ...
    if nodeIdx == prediction.nodeCount
        node.terminalContinuationAxis = "finitePredictionNode";
    end
