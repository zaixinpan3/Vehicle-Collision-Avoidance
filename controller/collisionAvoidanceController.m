...
% interface retains c; resetNominalTrajectory clears that interface.
    ...
    certificate = struct("version", 4, "plan", plan, ...
    ...
    metadata.longitudinalInputGain = cfg.model.longitudinalInputGain;
...
% from the new environment. It supplies no unverified fallback authority.
    related = isstruct(certificate) && isscalar(certificate) ...
        && all(isfield(certificate, ["version", "episodeIdentity", ...
            "plan", "schedule", "predictedState"])) && isequal(certificate.version, 4);
    if ~related
        return;
    end
...
function command = localCommand(inputPlan, model, prediction)
    ...
    axleNormalLoad = friction.staticNormalLoad ...
        + friction.normalLoadAccelerationSlope ...
            * cfg.model.longitudinalInputGain*longitudinalAcceleration;
    ...
    command.bodyLongitudinalVelocityDerivative = ...
        cfg.model.longitudinalInputGain*longitudinalAcceleration ...
        + model.longitudinalAccelerationBias+state(5)*state(6);
