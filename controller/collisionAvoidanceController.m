...
%collisionAvoidanceController One hard-safety, soft-CLF SOCP per sample.
% Optimize two held actuator inputs and one penalized CLF slack. There are
...
% output records the last input for slew constraints, not a future policy.
    ...
    predictedInput = result.decision(1:2);
    clfSlack = result.decision(3);
    ...
    metadata = struct('solverCallCount',1,'solverExitFlag',result.exitFlag, ...
        'solverMessage',result.message,'solverAlgorithm',"single hard-safety soft-CLF SOCP", ...
