...
% A solver-reported success is issued as returned; the plan is not re-verified
% after the solve. Subsequent frames optimize with the inherited normals fixed;
% a failed improvement can retain only the previously optimized continuation,
% and only after that carried witness passes the hard-safety check.
    ...
    % planCertified reports acceptance by solver status (or a retained,
    % checked incumbent); no post-solve recheck of the plan is performed.
    ...
    controllerState = struct('version',42, ...
