...
%collisionAvoidanceController Exact-state rolling MPC with a terminal witness.
...
% first scheduled affine hold. Future schedules are refreshed at every frame.
% Terminal feedback is a prediction certificate, never an executable fallback.
% No sensing radius or target exit is part of this mathematical plant study.
    ...
    if ~isempty(controllerState)
        [originalEncounter,model.initializationPlan,model.shiftedSafetyCandidate] = hardEncounterBarrier.validateTransition( ...
            controllerState,ego,model,observations,identity);
        model.horizonSteps = max(model.horizonSteps,size(model.initializationPlan,2));
    end
    ...
    if isempty(originalEncounter), originalEncounter = encounters; end
    ...
    certificate = struct("version", 18, "identity", identity, "stateTime", model.stateTime, ...
        ...
        "encounters", encounters, "originalEncounter", originalEncounter, "acceptance", check, "safetyScope", "certifiedPredictionWithHypotheticalInvariantTail", ...
    ...
    planningProblem.metadata.certificateCompatible = ~isempty(controllerState);
    planningProblem.metadata.shiftedSafetyCandidateChecked = planningTiming.shiftedCandidateChecked;
    planningProblem.metadata.carriedWitnessFeasible = planningTiming.shiftedCandidateAccepted;
