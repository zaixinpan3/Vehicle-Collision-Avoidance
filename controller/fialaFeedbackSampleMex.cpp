...
using Generators=std::array<std::array<double,12>,8>;
...
Tube initialTube(const double* lower,const double* upper,const double* nominal,const double* gain,const double* noise){
    Tube t;
    for(int i=0;i<6;++i){
        if(lower[i]>upper[i]||noise[i]<0)throw std::runtime_error("Invalid initial or measurement set.");
        t.center[i]=midpoint(I(lower[i],upper[i]));
        I offset=I(lower[i],upper[i])-I(t.center[i]);t.generators[i][i]=magnitude(offset);
    }
    for(int u=0;u<2;++u){
        I command(nominal[6+u]);
        for(int j=0;j<6;++j)command=command+I(gain[u+2*j])*(I(t.center[j])-I(nominal[j]));
        t.center[6+u]=midpoint(command);t.remainder[6+u]=command-I(t.center[6+u]);
        for(int j=0;j<12;++j){
            I coefficient=I(gain[u+2*(j%6)])*I(j<6?t.generators[j][j]:noise[j-6]);
            t.generators[6+u][j]=midpoint(coefficient);
            double radius=magnitude(coefficient-I(t.generators[6+u][j]));
            t.remainder[6+u]=t.remainder[6+u]+I(-radius,radius);
        }
    }return t;
}
bool enclose(const Tube& tube,double h,const double* parameters,Box& domain,Box& swept){
    Box inlet=outer(tube);Flow initialFlow=flow(state(inlet),parameters);
    for(int i=0;i<8;++i){
        if(i>=6){domain[i]=inlet[i];continue;}
        I probe=hull(inlet[i]+I(0,h)*initialFlow[i].v,I(tube.center[i]));
        double center=midpoint(probe);I r=I(magnitude(probe-I(center)))*I(1.5)+I(1e-10);
        domain[i]=I(center)+I(-r.hi,r.hi);
    }
    for(int iteration=0;iteration<10;++iteration){
        Flow f=flow(state(domain),parameters);bool inside=true;
        for(int i=0;i<8;++i){swept[i]=i<6?inlet[i]+I(0,h)*f[i].v:inlet[i];
            if(i<6)inside=inside && swept[i].lo>domain[i].lo && swept[i].hi<domain[i].hi;}
        if(inside)return true;
        for(int i=0;i<6;++i){I both=hull(domain[i],swept[i]);double center=midpoint(both);
            I r=I(magnitude(both-I(center)))*I(1.25)+I(1e-10);domain[i]=I(center)+I(-r.hi,r.hi);}
    }return false;
}
Tube propagate(const Tube& prior,const Box& domain,double duration,const double* parameters,Flow& residual){
    Flow field=flow(state(domain),parameters),atCenter=flow(state(pointBox(prior.center)),parameters);
    Matrix f{};Box offset{};
    for(int i=0;i<6;++i){
        ...
        for(int j=0;j<8;++j)value=value+(field[i].d[j]-f[i][j])*(domain[j]-I(prior.center[j]));
        residual[i]=D(value);
    }
    Maps transition=maps(f,duration);Tube next=prior;
    next.center=rk4(prior.center,duration,parameters);
...
mxArray* generatorsArray(const Generators& g){mxArray* a=mxCreateDoubleMatrix(8,12,mxREAL);double* x=mxGetDoubles(a);
    for(int i=0;i<8;++i)for(int j=0;j<12;++j)x[i+8*j]=g[i][j];
    return a;}
...
void mexFunction(int nlhs,mxArray* out[],int nrhs,const mxArray* in[]){
    try{
        if(nrhs!=9 || nlhs!=1)throw std::runtime_error("Nine arguments and one output are required.");
        ...
        const double* timing=vector(in[6],4);const double* previous=vector(in[7],2);
        ...
        double duration=timing[0],maxStep=timing[1],minStep=timing[2],maxCells=timing[3];
        ...
        Tube tube=initialTube(lower,upper,nominal,gain,noise);Box initial=outer(tube);
        bool accepted=true;std::string reason="complete";std::vector<Cell> cells;int rejected=0;double time=0;
        for(int u=0;u<2;++u){
            I command=initial[6+u];I difference=command-I(previous[u]);
        ...
        while(accepted && time<duration){
            ...
            while(!stepAccepted){
                I elapsed=I(end)-I(time);Box domain,swept;Flow residual;Tube candidate;
                try{
                    if(enclose(tube,elapsed.hi,p,domain,swept)){
                        ...
                        candidate=propagate(tube,domain,elapsed.hi,p,residual);
                        Flow derivative=flow(state(domain),p);
                        I gap=I(0,(I(elapsed.hi)-I(elapsed.lo)).hi);
                        for(int i=0;i<6;++i)candidate.remainder[i]=candidate.remainder[i]-gap*derivative[i].v;
        ...
        const char* fields[]={"accepted","reason","sampleTime","verifiedThrough","rejectedCells","initialBox","endpoint", "center","generators","remainder","cells"};
