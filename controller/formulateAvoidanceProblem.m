...
% the same bicycle, actuator, geometry and model-domain constraints.
    ...
    cruiseError = prediction.egoStateOffset(2:6, 1) ...
        - [0.0; 0.0; model.referenceSpeed; ...
            common.equilibrium.lateralVelocity(1); common.equilibrium.yawRate(1)];
    preferredInput = common.equilibrium.input;
    preferredInput(:, 1) = preferredInput(:, 1) ...
        - common.certificate.sampledFeedbackGain*cruiseError;
    [hessian, linear, constant] = localObjective(model, preferredInput, layout);
    qp = struct("problemClass", "certificatePreservingCbfClfQp", ...
        ...
        "preferredInput", preferredInput(:, 1), ...
...
% require positive relaxation; this reference certificate is local.
    ...
    try
        ...
        [sampledA, sampledB] = ltvBicycleModel.stageMatrices( ...
            0.0, key.referenceSpeed, key.sampleTime, cfg);
        [sampledFeedbackGain, ~, sampledPoles] = dlqr( ...
            sampledA(errorIndex, errorIndex), sampledB(errorIndex, :), ...
            key.sampleTime*stateWeight, key.sampleTime*inputWeight);
    ...
    certificate = struct( ...
        ...
        "sampledFeedbackGain", sampledFeedbackGain, ...
        "sampledClosedLoopPoles", sampledPoles, ...
