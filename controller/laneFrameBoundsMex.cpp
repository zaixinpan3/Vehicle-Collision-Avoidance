...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    const mwSize radiusCount = mxGetNumberOfElements(prhs[1]);
    if (segments == 0 || mxGetN(prhs[3]) != 2 || mxGetM(prhs[6]) != segments
            ...
            || (radiusCount != 1 && radiusCount != count) || mxGetNumberOfElements(prhs[2]) != 1) {
    ...
    const double* radii = mxGetDoubles(prhs[1]);
    ...
    if (lateralRadius < 0 || std::any_of(radii, radii+radiusCount,
    ...
    for (mwSize point = 0; point < count; ++point) {
        const double radius = radii[radiusCount == 1 ? 0 : point];
