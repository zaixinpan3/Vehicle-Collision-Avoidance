...
function ego = localReadEgoState(data, cfg)
    ...
    if longitudinalVelocity < cfg.model.speedMinimum ...
...
function target = localEmptyTarget()
    target = struct("position", zeros(2, 1), ...
        ...
        "predictionMotionBounds", [], "errorCertificate", []);
end
...
function [target, active] = localReadTargetRecord(data, ego, cfg)
    ...
    if isempty(target.errorCertificate)
        ...
        target.predictionAccelerationErrorBound = ...
            localOptionalNonnegativeInputVector( ...
                data, "targetPredictionAccelerationInertialErrorBound", 2);
        ...
        target.velocityErrorBound = values(3:4);
        target.accelerationErrorBound = values(5:6);
        target.yawErrorBound = values(7);
        target.yawRateErrorBound = values(8);
        target.predictionMotionBounds = localTargetMotionBounds(data);
        if isfinite(ego.stateTime)
            tolerance = 128*eps(max([1.0, abs(ego.stateTime), ...
                abs(target.errorCertificate.time)]));
            if abs(ego.stateTime-target.errorCertificate.time) > tolerance
                error("collisionAvoidanceController:staleEstimatorBound", ...
                    "The ego and target estimates must share a state timestamp.");
            end
        end
    end
    active = true;
...
function bounds = localTargetMotionBounds(data)
    fields = ["speedMaximum", "accelerationNormMaximum", ...
        "yawRateMaximum", "jerkNormMaximum"];
    if ~isfield(data, "targetPredictionMotionBounds") ...
            || ~isstruct(data.targetPredictionMotionBounds) ...
            || ~isscalar(data.targetPredictionMotionBounds) ...
            || ~all(isfield(data.targetPredictionMotionBounds, fields))
        error("collisionAvoidanceController:missingPredictionBound", ...
            "A current target bound requires declared future motion bounds.");
    end
    bounds = data.targetPredictionMotionBounds;
    for field = fields
        value = bounds.(field);
        if ~isnumeric(value) || ~isreal(value) || ~isscalar(value) ...
                || ~isfinite(value) || value < 0.0
            error("collisionAvoidanceController:invalidEstimatorBound", ...
                "Target motion bounds must be finite nonnegative scalars.");
        end
    end
end
