...
%
% The eight continuous core states per single target are body velocity and
...
% either chain. A circle set is propagated and intersected separately; its
% minimax representative is used only to publish inertial-frame outputs.
...
function runtime = localInitialize(cfg, options, observerDesign)
    ...
    runtime.positionErrorBound = nrmmPositionErrorBound("initialize", ...
        observerDesign,cfg,localBoundState(localPackState(runtime),targetCount,egoYaw),initialTime,prior);
end
...
function [runtime, output] = localStep(runtime, frame)
    ...
    runtime.yawEstimate = runtime.positionErrorBound.orientationSet.heading;
    runtime.targetCertifiedSpeedDomainValid = ...
        runtime.targetCertifiedSpeedDomainValid ...
...
function output = localCurrentOutput(runtime, frame)
    ...
    outputRuntime.yawEstimate = outputRuntime.positionErrorBound.orientationSet.heading;
...
function output = localOutput(runtime, observerInput)
    ...
    output = struct( ...
        ...
        "certificateScope",struct( ...
            ...
            "yawAssessment","output-only propagated/intersected set"), ...
...
function estimates = localTargetEstimates(runtime, observerInput)
    ...
    for targetIdx = 1:runtime.targetCount
        ...
        estimate.inertialRelativePositionErrorBound = radius ...
            +2*min(norm(estimate.relativePosition),runtime.positionErrorBound.trueRangeMaximum(targetIdx)) ...
            *sin(min(runtime.positionErrorBound.yaw,pi)/2);
        estimate.inertialOutputSet = struct("orientation",runtime.positionErrorBound.orientationSet, ...
            "bodyCenters",runtime.targetState(:,targetIdx), ...
            "bodyRadii",runtime.positionErrorBound.targetComponents(:,targetIdx), ...
            "trueNormBounds",[runtime.positionErrorBound.trueRangeMaximum(targetIdx); ...
...
function [bodyVelocity, position, gnssPositionPredictor, targetState, ...
    ...
    targetStateEnd = 6+6*targetCount;
    targetState = reshape(state(7:targetStateEnd), 6, targetCount);
    targetOutputPredictor = reshape(state((targetStateEnd+1):end), 2, targetCount);
end

function state = localBoundState(packed,targetCount,initialYaw)
    if nargin < 3
        initialYaw = 0;
    end
    [velocity,~,~,target,predictor] = localUnpackState(packed,targetCount);
    state = struct("yaw",initialYaw,"bodyVelocity",velocity, ...
