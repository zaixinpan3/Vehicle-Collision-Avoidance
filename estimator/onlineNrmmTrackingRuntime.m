...
function [bodyVelocity, position, gnssPositionPredictor, targetState, ...
        targetOutputPredictor, yaw] = localUnpackState(state, targetCount)
    ...
    targetOutputPredictor = reshape(state((targetStateEnd+1):(end-1)), 2, targetCount);
    yaw = state(end);
end

function state = localBoundState(packed,targetCount)
    [velocity,~,~,target,predictor,yaw] = localUnpackState(packed,targetCount);
