for n=["estimatorStraightNoRoadFixed","estimatorCircularNoRoadFixed"]
  ...
  pm=te.predictionMotion; jerk=zeros(2,1); if isfield(pm,'jerkBound'), jerk=pm.jerkBound; end % field of the recorded contract, removed 2026-09-24
  fprintf('contract %s jerk [%s] curvMax %.4f speedRateMax %.3f accMax %.3f\n',pm.kind,num2str(jerk'),pm.curvatureMaximum,pm.speedRateMaximum,pm.scalarAccelerationMaximum);
