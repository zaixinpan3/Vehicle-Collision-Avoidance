...
% ego-only tube; the nrmm-motion-v1 contract from the measurement box alone
% (nrmm-boxOnly); the nrmm-motion-v1 contract with the published NRMM parameter
% error bounds of a declarer whose velocity and acceleration errors are discs
% of the box radii (nrmm-egoOnly); and that contract with the target-reactive
% tube at strengths 30 and 100. It reports the largest support ||G' n||_1 + rho
% of the collision records at 1, 2, 3, 4 and 4.8 s, the exit-record support and
% the input reserves. The directions are the frame's nominal ones; no solve is
% involved.
    addpath("controller","config","scripts","estimator");
    ...
    for index = 1:size(specs,1)
        ...
        for scale = scales
            for variant = ["cartesian-egoOnly","nrmm-boxOnly","nrmm-egoOnly","nrmm-reactive30","nrmm-reactive100"]
                model = localCapture(scenario,roadCurvature,targetCurvature,scale,curvatureMaximum, ...
                    variant=="cartesian-egoOnly",variant~="nrmm-boxOnly",egoBound);
...
function model = localCapture(scenario,roadCurvature,targetCurvature,scale,curvatureMaximum,cartesian,publish,egoBound)
...
% publish adds the NRMM parameter error bounds of velocity and acceleration
% error discs of the box radii.
    ...
    if publish && ~cartesian
        parameters = nrmmTargetParameterErrorBounds(center(3:4),center(5:6),bound(3),bound(5),0);
        observation.targetSpeedErrorBound = parameters.speedErrorBound;
        observation.targetCourseErrorBound = parameters.courseErrorBound;
        observation.targetSpeedRateErrorBound = parameters.speedRateErrorBound;
        observation.targetCurvatureInterval = parameters.curvatureInterval;
    end
