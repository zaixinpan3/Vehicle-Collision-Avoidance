...
Latest implementation: [Removal of the Cartesian jerk contract](NRMM_CONTRACT_ONLY_20260924.md). At the user's direction `nrmm-motion-v1` is the only target motion contract: the `finite-sensing-motion-v1` Cartesian jerk contract, its default for targets without a descriptor, the per-hold acceleration part of the reactive tube, the whole-hold Cartesian target block of the geometry kernel and the estimator's Cartesian publication for a model error are removed (a target without a contract is refused). The declared-plant harness has only NRMM truths; the failure sweep replaces its jerk groups by NRMM curvature and speed-rate groups (120 cases, 108 pass; the 12 new cases pass; the 17 changed group-B rows are traced to the changed inputs with a pre-change control run). NRMM campaign 26/29/29 of 30, recursion 5 of 5 and estimator-bound 8 of 8 reproduce their records; 812 of 814 tests pass (2 data-dependent skips); the regenerated kernel reproduces a recorded run bitwise.
