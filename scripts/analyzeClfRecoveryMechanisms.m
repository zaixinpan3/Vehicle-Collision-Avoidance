...
function probe = localProbe(problem, model, sample)
    ...
    frictionIndices = find(qp.rowFamily == "friction");
    rowsPerStage = numel(frictionIndices)/prediction.stageCount;
    assert(rowsPerStage == floor(rowsPerStage));
    currentFriction = frictionIndices(1:rowsPerStage);
    assert(max(abs(qp.inequalityMatrix(currentFriction, 3:end-1)), [], 'all') < 1e-13);
    [physicalInput, ~, physicalFlag] = linprog(lateralInput.', ...
        qp.inequalityMatrix(currentFriction, 1:2), qp.inequalityBound(currentFriction), ...
        [], [], qp.lowerBound(1:2), qp.upperBound(1:2), options);
    assert(physicalFlag > 0);
    physicalResidual = qp.inequalityMatrix(currentFriction, 1:2)*physicalInput ...
        - qp.inequalityBound(currentFriction);
    inverseP = p\eye(5);
    probe = struct('timeSeconds', sample.timeSeconds, 'recorded', sample, ...
        ...
        'currentFrictionRowCount', numel(currentFriction), ...
