...
% range.
    ...
    results.terminalInvariantCertified = true(stepCount, 1);
    results.terminalInvariantMargin = inf(stepCount, 1);
    ...
    for stepIdx = 1:stepCount
        ...
        try
            if lightweight
                ...
                results.terminalInvariantCertified(stepIdx) = ...
                    md.terminalInvariantCertified;
                results.terminalInvariantMargin(stepIdx) = ...
                    md.terminalInvariantMargin;
...
function localReport(results, sampleTime, cfg)
    ...
    if any(visible)
        ...
        fprintf(['terminal set: tail margin min %.4f m over %d ' ...
            ...
            sum(results.terminalInvariantCertified(visible)), sum(visible), ...
            min(results.terminalInvariantMargin(visible)), ...
    ...
    fprintf(['CLF relaxation: max %.4g (first-step relaxation), ' ...
    ...
    fprintf('; exact decrease residual max %.3g\n', ...
