...
% Optional prior affine-certificate audit is diagnostic, not a controller run.
    ...
    for index=1:numel(scales)
        timer=tic;
        certificates{index}=fialaCertificate.residual(center-scales(index)*radius,center+scales(index)*radius,a,b,c,cfg);
    ...
    cellCertificate=fialaCertificate.residual(center-domainRadius,center+domainRadius,a,b,c,cfg,feedback);
    assert(cellCertificate.heldCell.accepted);
    feedback.duration=.5;
    rejected=fialaCertificate.residual(center-domainRadius,center+domainRadius,a,b,c,cfg,feedback);
    ...
    if strlength(priorGeometryDirectory)>0
        ...
        for stage=1:prediction.stageCount
            ...
            domain=fialaCertificate.residual(lower,upper,aa,bb,cc,model.cfg);
            point=[points(:,1);command];pointCheck=fialaCertificate.residual(point,point,aa,bb,cc,model.cfg);
