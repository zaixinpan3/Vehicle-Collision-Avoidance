...
% certificate does not cover; it is recorded, not repaired.
    ...
    cases = localCases();
    columns = ["group", "scenario", "curvature", "uncertaintyScale", "targetCurvature", "targetSpeedRate", ...
    ...
    for index = 1:numel(cases)
        ...
        rows(index, :) = {c.group, c.Scenario, c.RoadCurvature, c.uncertaintyScale, c.targetCurvature, ...
            c.targetSpeedRate, c.initialLateral, c.initialHeading, c.initialSpeed, c.roadBoundaries, ...
        ...
        for curvature = [0, .01]
            for scale = [1, 3, 10]
                item = localCase("B-uncertainty", scenario, curvature, {"EgoErrorBound", scale*egoBase, ...
                    "TargetErrorBound", scale*targetBase});
                item.uncertaintyScale = scale;
                cases{end+1} = item; %#ok<AGROW>
            end
        end
    end
    for scenario = ["oncoming", "crossing"]
        % NRMM truths on curved paths and with speed changes, measured with
        % the nominal noise and declared under the default 0.05 1/m maximum.
        for targetCurvature = [.005, .01, .02]
            item = localCase("C-targetCurvature", scenario, 0, {"TargetErrorBound", targetBase, ...
                "TargetCurvature", targetCurvature});
            item.targetCurvature = targetCurvature; cases{end+1} = item; %#ok<AGROW>
        end
        for speedRate = [-.5, .5, 1.0]
            item = localCase("C-targetSpeedRate", scenario, 0, {"TargetErrorBound", targetBase, ...
                "TargetSpeedRate", speedRate});
            item.targetSpeedRate = speedRate; cases{end+1} = item; %#ok<AGROW>
        end
    end
...
function item = localCase(group, scenario, curvature, extra)
    item = struct("group", group, "Scenario", scenario, "RoadCurvature", curvature, "extra", {extra}, ...
        "uncertaintyScale", 0, "targetCurvature", 0, "targetSpeedRate", 0, "initialLateral", 0, ...
