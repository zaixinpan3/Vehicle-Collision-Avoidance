function result = runRobustVelocityCertificateScenario()
%runRobustVelocityCertificateScenario Validate a complete uncertain-state continuation.
% All 64 vertices of a six-state enclosure receive the same held inputs.
% Only initial admission succeeds; every subsequent solver attempt fails.
% The 60-second continuation verifies containment, clearance and dissipation
% under the declared affine dynamics, not the nonlinear physical plant.
    ...
    radius = [0.04; 0.04; 0.014; 0.388; 0.388; 0.0015];
    ...
    count = round(60/cfg.controller.sampleTime);
    signs = 2*double(dec2bin(0:63, 6).'-'0')-1;
    truth = stored.predictedState(:, 1)+radius.*signs;
    minClearance = inf;
    maxContainmentResidual = -inf;
    fallback = false(1, count);
    estimateBound = zeros(6, count);
    effectiveBound = zeros(6, count);
    maximumVelocityMagnitude = zeros(3, count+1);
    maximumVelocityMagnitude(:, 1) = max(abs(truth(4:6, :)), [], 2);
    for step = 1:count
        if step == 1
            problem = first;
        else
        ...
        maximumVelocityMagnitude(:, step+1) = max(abs(truth(4:6, :)), [], 2);
    end
    result = struct("scope", "declaredAffineModelPredictionNodes", ...
        "scenario", "straight-road-64-state-vertices", "sampleCount", count, ...
