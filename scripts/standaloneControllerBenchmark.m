...
% joint-certificate frames are exported; this is not a deployment interface.
    methods (Static)
        ...
        function [program,result,search]=nativeJoint(program,~,cfg)
            ...
            elapsed=toc(start);called=double(metrics(2)>0);
            restored=double(status==4 || (status==0 && called>0));
            search=struct('hardSolves',called-restored,'restorationSolves',restored,'baseSolves',0,'nativeSolves',called, ...
                'familyAttempts',double(status==1 || restored>0),'horizonAttempts',1, ...
                ...
                'issuedAdmissionWitness',status==1 || status==4,'usedFullPlanAdmission',status==4, ...
                'initialCertificateAngles',initialAngles, ...
                'nativeKernel',true,'nativePhaseSeconds',metrics);
            if restored>0,search.policy="affineSectionThenJointAdmission";end
