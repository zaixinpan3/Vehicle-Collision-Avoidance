...
function [problem, cfg] = localProblem(referenceRate)
    cfg = collisionAvoidanceControllerConfig(struct("referenceSpeed",3,"solver",struct("programForm","lifted"),"controller", struct("horizonSteps",8,"sampleTime",0.1,"stationTrustRadius",5), ...
...
% residual is concave and peaks strictly between the two sample endpoints.
    cfg = collisionAvoidanceControllerConfig(struct("referenceSpeed",0,"solver",struct("programForm","lifted"), ...
    ...
    prediction = struct("stageCount",1,"nodeCount",2,"planCount",2, ...
        ...
        "stageMatrixA",exact(1:6,1:6),"stageMatrixB",exact(1:6,7:8),"stageAffine",exact(1:6,9), ...
        "initialErrorBound",zeros(6,2));
