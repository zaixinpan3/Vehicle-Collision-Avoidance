classdef jointSupportCertificateTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function conditioningOneUncertainTargetPreservesItsCertificate(testCase)
            ...
            testCase.verifyTrue(first.metadata.planCertified);
            testCase.verifyEqual(next.model.encounter.key, ...
...
function record=localRecord()
    record=struct('stage',1,'isExit',false,'positionMap',[eye(2),zeros(2,4)], ...
...
function [program,cfg]=localSquare()
    ...
    geometry=struct('label',strings(0,1),'local',struct('stage',{}),'physicalBound',zeros(0,1), ...
        'normals',{{[1;0]}},'cellData',struct('normal',[1;0]),'frames',struct('heading',0));
